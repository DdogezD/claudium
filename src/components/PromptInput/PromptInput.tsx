...
function PromptInput({
  ...
  // Effective selection: null if the selected pill stopped rendering.
  ...
  const combinedHighlights = useMemo((): TextHighlight[] => {
    ...
    return highlights;
  }, [isSearchingHistory, historyQuery, historyMatch, historyFailedMatch, cursorOffset, btwTriggers, imageRefPositions, memberMentionHighlights, slashCommandTriggers, tokenBudgetTriggers, slackChannelTriggers, displayedValue, voiceInterimRange, buddyTriggers]);
  const {
    addNotification,
    removeNotification
  } = useNotifications();
  ...
  if (showBashesDialog) {
    return <BackgroundTasksDialog onDone={() => setShowBashesDialog(false)} initialDetailTaskId={typeof showBashesDialog === 'string' ? showBashesDialog : undefined} />;
  }
