...
Hooks run commands at specific points in Claudium's lifecycle.
...
| Stop | - | Run when the session stops (including clear, resume, compact) |
...
Modify Claudium configuration by updating settings.json files.
...
- \`theme\`, \`editorMode\`, \`verbose\`, \`modelProfiles.main.model\`
...
User: "Format my code after the assistant writes it"
...
6. **Use --debug** - Run with \`--debug\` flag to see hook execution logs
`
